...
pay_operation({"method": "get_client_id"}).then((payload) => { console.log(payload);});
//new fetch code goes
 ...
 */
let paymentsClient = null, google_pay_config = null;
/**
 * 
...
async function getGooglePaymentDataRequest() {
  const {allowedPaymentMethods,merchantInfo, apiVersion, apiVersionMinor , countryCode} = await get_google_pay_config();
  ...
  function onPaymentAuthorized(paymentData) {
    return new Promise(function(resolve, reject) {
      processPayment(paymentData)
        ...
        .catch(function() { resolve({transactionState: 'ERROR'});
        });
    });
  }
/**
...
async function onGooglePayLoaded() {
  ...
  const { allowedPaymentMethods, apiVersion, apiVersionMinor } = await get_google_pay_config();
...
async function processPayment(paymentData) {
  return new Promise(async function (resolve, reject) {
    try {
      const order_request = await pay_operation({ "method": "order" });
      let { id } = await order_request.json();
      const confirmOrderResponse = await paypal.Googlepay().confirmOrder({ orderId: id, paymentMethodData: paymentData.paymentMethodData });
      if (confirmOrderResponse.status === "APPROVED") {
        const response = await pay_operation({ "method": "complete", "order_id": id });
...
async function setup_apple_pay() {
  ...
  async function handle_apple_pay_click() {
    const paymentRequest = { countryCode, currencyCode: 'USD', merchantCapabilities, supportedNetworks,
    ...
    apple_pay_session.onpaymentauthorized = async (event) => {
      try {
        const apple_pay_order_request = await pay_operation({"method": "order"});
        ...
        /* Confirm Payment */
        await applepay.confirmOrder({ orderId: order_id, token: event.payment.token, billingContact: event.payment.billingContact , shippingContact: event.payment.shippingContact });
        pay_operation({"method": "complete", "order_id": order_id}).then(apple_complete_request => {
